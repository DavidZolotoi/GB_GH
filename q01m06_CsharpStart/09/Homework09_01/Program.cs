...
System.Console.WriteLine($"Четные числа в диапазоне с {range.start} по {range.finish}: {recurs(range)}");

// Рекурсивный способ выведения четных чисел в промежутке
string recurs((int start, int finish) range, string outputText="")
{
