...
public class hw03_02
{
    ...
    private static LinkedList<Integer> DelEvenValue(LinkedList<Integer> integerList, Logger logger)
    {
        ...
        while(i < integerList.size())
        {
            ...
            {
                logger.warning("Найдено четное число для удаления: " + integerList.get(i));
    ...
    {
        // сам логгер
        Logger logger = Logger.getLogger(hw03_02.class.getName());
        try
        {
    ...
    private static String GetPathForLogFile(String fileName)
    {
        ...
        {
            // путь к файлу
            String pathFileJava = Class.forName("hw03_02").getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
