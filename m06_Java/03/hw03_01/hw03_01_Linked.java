...
/* !!! НЕ ПОЛУЧИЛОСЬ, ПОЧЕМУ? Читать ниже.
 * Пытался сделать через связный список - плохая идея,
 * потому что в рекурсии идет работа со ссылками, и новые листы не создаются.
 * Поэтому чтоб решить подобную задачу через рекурсию,
 * нужно продумать алгоритм нового создания списков при каждом рекурсивном вызове метода
 */

public class hw03_01_Linked
{
                                ...
                                LinkedList<Integer> newListRight)
    {
        ...
        for (int i = indexLeft; i < listLeftCount; i++)
            integerList.add(indexList++, newListLeft.get(i));
        for (int i = indexRight; i < listRightCount; i++) 
            integerList.add(indexList++, newListRight.get(i));
    }
    ...
    {
        // сам логгер
        Logger logger = Logger.getLogger(hw03_01_Linked.class.getName());
        try
        {
    ...
    private static String GetPathForLogFile(String fileName)
    {
        ...
        {
            // путь к файлу
            String pathFileJava = Class.forName("hw03_01_Linked").getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
