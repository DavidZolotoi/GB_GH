...
public class hw04_01
{
    public static void main(String[] args)
    {
        ...
        System.out.printf("Обработанный \"перевёрнутый\" массив: \n%s\n", OutputList(GetReverse(integerList, logger), "  ", logger));
    
        logger.info("Конец работы программы.");
    }
    
    // Возврат реверсного листа
    private static LinkedList<Integer> GetReverse(LinkedList<Integer> integerList, Logger logger)
    {
        logger.info("Начало работы метода возврата реверсного листа.");
        for (int i = 0; i < integerList.size() / 2; i++)
        {
            Integer tmp = integerList.get(integerList.size() - i - 1);
            integerList.set(integerList.size() - i - 1, integerList.get(i));
            integerList.set(i, tmp);
        }

        logger.info("Конец работы метода возврата реверсного листа.");
        return integerList;
    }
    ...
    {
        // сам логгер
        Logger logger = Logger.getLogger(hw04_01.class.getName());
        try
        {
    ...
    private static String GetPathForLogFile(String fileName)
    {
        ...
        {
            // путь к файлу
            String pathFileJava = Class.forName("hw04_01").getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
